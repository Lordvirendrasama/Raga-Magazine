...
export default function LivePage() {
  ...
  const [categoryName, setCategoryName] = useState('Live');

  useEffect(() => {
    async function fetchLivePosts() {
      setLoading(true);
      try {
        const liveCategory = await getCategoryBySlug('live');
        
        if (!liveCategory) {
          console.error("The 'live' category was not found in your WordPress site.");
          setPosts([]);
          setLoading(false);
